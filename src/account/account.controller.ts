...
const addressSchema = z.object({ label: z.string().trim().min(2).max(50), recipient: z.string().trim().min(2).max(100), phone: z.string().trim().min(8).max(20), line1: z.string().trim().min(5).max(200), line2: z.string().trim().max(200).optional().or(z.literal("")), district: z.string().trim().min(2).max(100), city: z.string().trim().min(2).max(100), province: z.string().trim().min(2).max(100), postalCode: z.string().regex(/^\d{5}$/), country: z.string().trim().min(2).max(80).default("Indonesia"), isDefault: z.boolean().default(false) });
